...
REPETITIONS=2         # Default value 1

# Experiment group name
EXPERIMENT_GROUP="fast_optimizer"

# List of optimizers
OPTIMIZERS=("SGD" "KFAC" "Adam")
...
run_experiment() {
    ...
    nohup python3 ../breaching/tilo_experiment.py $GPU_INDEX $MAX_ITERATIONS $optimizer $MODEL $experiment_name --callback_interval $CALLBACK_INTERVAL --batch_size $batch_size --repetitions $REPETITIONS --epoch_count 4 --run_attacks_during_training=false > ${experiment_name}.out 2>&1 &
